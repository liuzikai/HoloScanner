...
#endif
{
...
#ifdef USE_DBSCAN
    std::vector<size_t> index = std::get<1>(pcd.RemoveStatisticalOutliers(16, 0.4));
    pcd = *pcd.SelectByIndex(index);
    std::vector<int> labels = pcd.ClusterDBSCAN(0.005, 16);
    ...
    size_t ulSize = labels_unique.size();
    std::vector<size_t> labels_num(ulSize, 0);
    std::vector<double> distances(ulSize, std::numeric_limits<double>::max());
